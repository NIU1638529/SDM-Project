    ...
    for event, elem in context:
        if current_tag is None and event == 'start' and elem.tag in elements:
            ...
            if elem.tag == current_tag:
                current_tag = None
                current_tag = None
                limit_count+= 1              # <--- AÑADE ESTO
                if limit_count >= 100000:     # <--- AÑADE ESTO
                    break                       # <--- AÑADE ESTO
...
def get_element_attributes(xml_file, elements: set) -> dict:
    ...
    # RECORRIDO COMPLETO: Sin límites para encontrar todas las columnas (ee-type, etc.)
    for event, elem in context:
        if current_tag is None and event == 'start' and elem.tag in elements:
            current_tag = elem.tag
            keys = elem.keys()
